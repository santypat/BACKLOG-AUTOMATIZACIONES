...
def obtener_tareas():
    ...
    try:
        ...
        if not rel_df.empty and not devs_df.empty:
            rel_df = rel_df.merge(devs_df, left_on="dev_id", right_on="id", suffixes=('_rel', '_dev'))
            ...
            # Unir con tareas
            df = df.merge(rel_grouped, left_on="id", right_on="desarrollo_id", how="left")
...
def finalizar_tarea(id, horas_opt, descripcion):
    """Finaliza una tarea con datos de optimización"""
    try:
        supabase.table("desarrollos").update({
            "estado": "Terminado",
